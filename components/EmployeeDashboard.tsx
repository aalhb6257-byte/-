...
const EmployeeDashboard: React.FC<EmployeeDashboardProps> = ({ currentUser, activityLogs, users, onTabChange }) => {
  ...
  const personalLogs = activityLogs.filter(log => currentUser?.name && log.details.includes(currentUser.name)).slice(0, 5);

  return (
    <div className="space-y-10 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black text-black tracking-tight dark:text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass-panel p-8 lg:col-span-2 flex flex-col md:flex-row gap-8 items-center md:items-start border border-white/10">
          ...
          <div className="flex-1 text-center md:text-right">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div>
                <h3 className="text-2xl font-black text-black dark:text-white">{currentUser?.name}</h3>
              ...
              <button 
                ...
                className="px-6 py-2.5 liquid-btn text-slate-800 dark:text-white rounded-xl font-black text-xs transition-all"
              >
                {t.edit} {t.profile}
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 text-right">
              <div>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest mb-1">{t.department}</p>
                <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{userDetails?.department || '---'}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest mb-1">{t.phone}</p>
                <p className="text-sm font-bold text-slate-700 dark:text-slate-300" dir="ltr">{currentUser?.phone}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest mb-1">{t.email}</p>
                <p className="text-sm font-bold text-slate-700 dark:text-slate-300 truncate">{currentUser?.email}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest mb-1">{t.salary}</p>
                <p className="text-sm font-bold text-emerald-400">{userDetails?.salary?.toLocaleString() || '---'} <span className="text-[10px]">د.ع</span></p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest mb-1">{t.joinDate}</p>
                <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{userDetails?.appointmentDatePermanent || '---'}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest mb-1">{t.status}</p>
        ...
        <div className="space-y-6">
          <div className="glass-panel p-8 bg-gradient-to-br from-indigo-600/50 to-indigo-700/50 text-white border border-white/10 shadow-xl shadow-indigo-600/20">
          ...
          <div className="glass-panel p-6 border border-white/10">
            <h4 className="text-sm font-black text-black dark:text-white mb-4 uppercase tracking-widest">
              {t.recentActivity}
            </h4>
      ...
      <div className="glass-panel p-8 border border-white/10">
        <h3 className="text-lg font-black text-black dark:text-white mb-4">{t.biography}</h3>
